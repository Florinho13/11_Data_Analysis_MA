...
fit_one_field <- function(long_df, var){
...
fit_all_vars_field <- function(df, domain_label){
  long_df <- to_long(df)
  vars <- long_df %>% dplyr::distinct(variable) %>% dplyr::pull(variable)
  res <- purrr::map_dfr(vars, ~fit_one_field(long_df, .x)) %>%
    dplyr::mutate(
      domain = domain_label,
...
bio_res_field  <- fit_all_vars_field(soil_biological_param,  "biological")
chem_res_field <- fit_all_vars_field(soil_chemical_param, "chemical")
phys_res_field <- fit_all_vars_field(soil_physical_param, "physical")



plant_res_field <- fit_all_vars_field(plant_health_param, "plant") %>% 
  mutate(model = "field_as_random", .after = 2)

all_res_field <- bind_rows(bio_res_field, chem_res_field, phys_res_field) %>% 
  mutate(model = "field_as_random", .after = 2)

phi_res_field <- fit_all_vars_field(phi_total[,-c(2:4)],"phi") %>% 
  mutate(model = "field_as_random", .after = 2)

shi_res_field <- fit_all_vars_field(scored_soil_data_rq_total[,-c(2:4)],"shi") %>% 
  mutate(model = "field_as_random", .after = 2)
